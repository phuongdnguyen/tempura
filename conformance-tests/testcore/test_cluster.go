...
var buildTempuraOnce sync.Once
...
type (
	...
	TestCluster struct {
		...
		proxyPort             int
		adminPort             int
		tempuraFrontendClient workflowservice.WorkflowServiceClient
	}
...
func newClusterWithPersistenceTestBaseFactory(
	...
	buildTempuraOnce.Do(func() {
		projectRoot, _ := filepath.Abs("..")
		buildCmd := exec.Command("go", "build", "-o", "tempura_test_bin")
		buildCmd.Dir = projectRoot
		buildCmd.Stdout = os.Stdout
		buildCmd.Stderr = os.Stderr
		if err := buildCmd.Run(); err != nil {
			logger.Fatal("Failed to build tempura", tag.Error(err))
		}
	})
	...
	proxyPort := freeport.MustGetFreePort()
	adminPort := freeport.MustGetFreePort()
	projectRoot, _ := filepath.Abs("..")

	tempuraCmd := exec.Command(filepath.Join(projectRoot, "tempura_test_bin"),
		"-redis-addr", mr.Addr(),
		"-listen-addr", fmt.Sprintf("localhost:%d", proxyPort),
		"-admin-port", fmt.Sprintf("%d", adminPort),
		"-default-target", "http://"+cluster.FrontendGRPCAddress(),
	)
	tempuraCmd.Dir = projectRoot
	tempuraCmd.Stdout = os.Stdout
	tempuraCmd.Stderr = os.Stderr
	if err := tempuraCmd.Start(); err != nil {
		return nil, err
	}

	time.Sleep(2 * time.Second) // Wait for proxy to start

	conn, err := grpc.Dial(fmt.Sprintf("localhost:%d", proxyPort), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return nil, err
	}
	tempuraFrontendClient := workflowservice.NewWorkflowServiceClient(conn)
	...
	return &TestCluster{
		...
		host:                  cluster,
		miniredis:             mr,
		tempuraCmd:            tempuraCmd,
		proxyPort:             proxyPort,
		adminPort:             adminPort,
		tempuraFrontendClient: tempuraFrontendClient,
	}, nil
}
...
func (tc *TestCluster) TearDownCluster() error {
	...
	if tc.tempuraCmd != nil && tc.tempuraCmd.Process != nil {
		tc.tempuraCmd.Process.Kill()
	}
...
func (tc *TestCluster) FrontendClient() workflowservice.WorkflowServiceClient {
	if tc.tempuraFrontendClient != nil {
		return tc.tempuraFrontendClient
	}
	return tc.host.FrontendClient()
}

func (tc *TestCluster) TempuraProxyGRPCAddress() string {
	return fmt.Sprintf("localhost:%d", tc.proxyPort)
}

func (tc *TestCluster) TempuraAdminAPIAddress() string {
	return fmt.Sprintf("http://localhost:%d", tc.adminPort)
}
