...
func (s *FunctionalTestBase) FrontendGRPCAddress() string {
	return s.GetTestCluster().TempuraProxyGRPCAddress()
}
...
func (s *FunctionalTestBase) registerNamespaceWithTempura(nsName namespace.Name, nsID string) {
	adminAPI := s.testCluster.TempuraAdminAPIAddress()
	resp, err := http.Post(adminAPI+"/namespace/virtual/create", "application/json", strings.NewReader(`{"name":"`+nsName.String()+`"}`))
	if err == nil {
		resp.Body.Close()
	}

	clusterAddr := s.testCluster.Host().FrontendGRPCAddress()
	payload := fmt.Sprintf(`{"virtual_namespace":"%s", "physical_namespace":"%s", "cluster_address":"%s"}`, nsName.String(), nsName.String(), clusterAddr)
	resp, err = http.Post(adminAPI+"/namespace/physical/add", "application/json", strings.NewReader(payload))
	if err == nil {
		resp.Body.Close()
	}
}
...
func (s *FunctionalTestBase) RegisterNamespace(
	...
	s.registerNamespaceWithTempura(nsName, nsID.String())
